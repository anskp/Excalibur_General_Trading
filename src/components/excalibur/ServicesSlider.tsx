...
const colorStyles: Record<string, { icon: string; badge: string; border: string }> = {
  crimson: {
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-end justify-between mb-12">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <button
              onClick={() => scroll('left')}
              className="w-10 h-10 rounded-full glass border border-border flex items-center justify-center hover:border-gold/30 transition-colors"
        ...
        <div
          ref={scrollRef}
          className="flex gap-4 overflow-x-auto no-scrollbar scroll-smooth-horizontal pb-4 -mx-4 px-4 sm:mx-0 sm:px-0"
        >
          {services.map((service, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`h-full rounded-3xl glass-card hover:glass-card-hover border ${styles.border} p-6 transition-all duration-500 hover:-translate-y-1 cursor-pointer`}
                >
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider ${styles.badge}`}
                    >
                      {service.category}
                    </span>
                  </div>
