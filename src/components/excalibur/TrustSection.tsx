          ...
          {certifications.map((cert, index) => {
            ...
            return (
              <ScrollReveal key={cert.title} delay={index * 0.1}>
                <div
                  className={`group h-full rounded-3xl glass-card hover:glass-card-hover border ${styles.border} p-6 text-center transition-all duration-500 hover:-translate-y-1 hover:shadow-lg card-shine`}
                >
                  <div
                    className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl ${styles.icon} mb-5 transition-transform duration-300 group-hover:scale-110`}
                  >
          ...
          <div className="glass rounded-3xl p-8 lg:p-10 border border-gold/10">
            ...
            >
              {trustIndicators.map((indicator) => {
                ...
                return (
                  <StaggerItem key={indicator.label}>
