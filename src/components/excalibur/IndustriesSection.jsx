...
const colorMap = {
    gold: {
        icon: 'bg-white/10 text-gold-light',
        badge: 'bg-white/10 text-gold-light',
        border: 'border-white/10 hover:border-gold',
        accent: 'bg-gold',
    },
    olive: {
        icon: 'bg-white/10 text-gold-light',
...
export function IndustriesSection() {
    return (<section id="industries" className="relative py-24 lg:py-32 bg-charcoal">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <StaggerContainer staggerDelay={0.06} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-5">
          {industries.map((industry) => {
            const styles = colorMap[industry.color];
            const Icon = industry.icon;
            return (<StaggerItem key={industry.title}>
                <div className={`group relative h-full rounded-3xl bg-white/8 border ${styles.border} p-6 transition-all duration-500 hover:-translate-y-2 hover:shadow-lg card-shine cursor-pointer overflow-hidden`}>
                  {/* Top accent bar */}
                  <div className={`absolute top-0 left-6 right-6 h-px rounded-full ${styles.accent} opacity-0 group-hover:opacity-20 transition-opacity duration-500`}/>
