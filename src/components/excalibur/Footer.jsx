...
export function Footer() {
    ...
    return (<footer className="relative bg-charcoal text-ivory">
      {/* Top gold line */}
          ...
          {Object.entries(footerLinks).map(([title, links]) => (<div key={title}>
              ...
              <ul className="space-y-3">
                {links.map((link) => (<li key={link.label}>
                    <Link href={link.href} className="text-sm text-ivory/40 hover:text-ivory transition-colors duration-300">
                      {link.label}
                    </Link>
                  </li>))}
              </ul>
      ...
      <div className="border-t border-ivory/8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={scrollToTop} className="w-8 h-8 rounded-full bg-ivory/8 hover:bg-gold/20 flex items-center justify-center transition-colors" aria-label="Scroll to top">
